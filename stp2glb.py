...
for dft, dftData in types_of_defects.items():
    ...
    if(type(dftData) == MultiPolygon):
        ...
        for i in range(len(dftData.geoms)):
            ...
            for j in range(vertices.shape[0]):
                vs.append("v " + str((vertices[j][0])/1000) + " " + str((vertices[j][1])/1000) + " " + str((vertices[j][2])/1000) + "\n") #ran into scaling issues. Divided coords by 1000
    ...
    #f = open("curvedtool_originalcad_originzero_scaleFIXby1000_obj.obj",'w')
    f = open("curvedtool_" + str.lower(dft) + "_obj.obj", 'w')
    ...
    ##write .glb file from .obj
    os.system("obj2gltf -i curvedtool_" + str.lower(dft) + '_obj.obj -o curvedtool_' + str.lower(dft) + "_glb.glb")
########################################################################################################################3

##################################Copy/Paste to make AllDefectsFile######################################
faces_all = []
v_all = []
for dft, dftData in types_of_defects.items():
    vs = []
...
f = open("curvedtool_alldefects_obj.obj", 'w')
