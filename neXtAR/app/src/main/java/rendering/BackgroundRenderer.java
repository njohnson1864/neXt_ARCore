...
public class BackgroundRenderer {
  ...
  public void createOnGlThread(Context context, int depthTextureId) throws IOException {
    ...
    {
      int vertexShader =
          rendering.ShaderUtil.loadGLShader(TAG, context, GLES20.GL_VERTEX_SHADER, CAMERA_VERTEX_SHADER_NAME);
      int fragmentShader =
          rendering.ShaderUtil.loadGLShader(
      ...
      rendering.ShaderUtil.checkGLError(TAG, "Program creation");

      cameraTextureUniform = GLES20.glGetUniformLocation(cameraProgram, "sTexture");
      rendering.ShaderUtil.checkGLError(TAG, "Program parameters");
    }
  ...
  private void draw(boolean debugShowDepthMap) {
    ...
    rendering.ShaderUtil.checkGLError(TAG, "BackgroundRendererDraw");
  }
